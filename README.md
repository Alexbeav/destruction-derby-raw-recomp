...
[![GitHub downloads (all assets, all releases)](https://img.shields.io/github/downloads/Alexbeav/destruction-derby-raw-recomp/total)](https://github.com/Alexbeav/destruction-derby-raw-recomp/releases)
[![GitHub downloads (latest release)](https://img.shields.io/github/downloads/Alexbeav/destruction-derby-raw-recomp/latest/total)](https://github.com/Alexbeav/destruction-derby-raw-recomp/releases/latest)
[![GitHub release](https://img.shields.io/github/v/release/Alexbeav/destruction-derby-raw-recomp)](https://github.com/Alexbeav/destruction-derby-raw-recomp/releases/latest)
<!-- /retcomm-readme-metrics -->

Static recompilation of **Destruction Derby Raw** built on
[psxrecomp](https://github.com/mstan/psxrecomp) and
[recomp-ui](https://github.com/RetroPortingToolKit/recomp-ui).

This is an owned-input setup kit. You supply your own SLUS-00912 disc image
...
|---|---|
| Region | USA |
| Disc serial | SLUS-00912 |
| Discs | 1 |
| Supported dump | Redump `Destruction Derby Raw (USA).cue` as CUE/BIN or CHD |
| BIOS | SCPH-1001 (USA), 524288 bytes, SHA-256 `71af94d1e47a68c11e8fdb9f8368040601514a42a5a399cda48c7d3bff1e99d3` |

## Setup

1. Download the setup ZIP for your platform from [Releases](https://github.com/Alexbeav/destruction-derby-raw-recomp/releases)
   and extract it into a writable folder.
2. Start `Destruction_Derby_Raw` (`.exe` on Windows).
...
- Windows play testing on the private build this kit derives from: Playable, with frame-buffer artifacts in the menu and movies. The interlaced-field preservation correction is in the pinned framework and was accepted on a test candidate.
...
- This disc has audio tracks. Select the CUE (or a CHD made from the full dump) so every track is mounted; a Track 01-only BIN fails verification.
...
named `diagnostic-mode.txt` next to `Destruction_Derby_Raw.exe` and start the game as
usual, or run `Destruction_Derby_Raw.exe --diagnostic`. Reproduce the problem, quit, then
run `Destruction_Derby_Raw.exe --collect-diagnostics`: it writes `diagnostics-<date>.zip`
