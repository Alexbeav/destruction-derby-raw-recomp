...
#
# Writes: dist/destructionderbyraw-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix destructionderbyraw \
  --exe-name Destruction_Derby_Raw \
  --omit-openbios \
  --display-name "Destruction Derby Raw" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Destruction Derby Raw (USA) disc" \
  --runtime-dir mods \
